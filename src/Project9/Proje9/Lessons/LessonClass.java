...
//-------------------------------------------------------------------------------------------------------
    ...
    public LessonClass(StudentClass student)throws Exception {
   ...
   /*
    ...
    public void setCreditCount() throws Exception{
        ...
        while (totalCreditCount <= 7){
            ...
            if (lessonsName.contains(dersAdi)){
                System.out.println("You already took this class please choose other class");
            }else{
                lessonsName.add(dersAdi);
            }
            totalCreditCount+= checkCredit(dersAdi);
        }
    }
    /*
...
//-------------------------------------------------------------------------------------------------------
    ...
    public int checkCredit(String lessonName) throws Exception {
        ...
        if (lessonName.equalsIgnoreCase("Math")){
            ...
            totalCreditCount= 3;
        }if (lessonName.equalsIgnoreCase("Science")) {
            classNameAndScore.put(lessonName, RandomNum());
            totalCreditCount= 4;
        }if (lessonName.equalsIgnoreCase("Biology")) {
            classNameAndScore.put(lessonName, RandomNum());
            totalCreditCount= 2;
        }if (lessonName.equalsIgnoreCase("Chemistry")) {
            classNameAndScore.put(lessonName, RandomNum());
            totalCreditCount= 2;
        }if (lessonName.equalsIgnoreCase("Music")) {
            classNameAndScore.put(lessonName, RandomNum());
            totalCreditCount= 3;
        }else try{
            throw new Exception("You class name is not a valid class name.");
